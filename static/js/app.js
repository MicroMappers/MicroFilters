	...
	function(){
		$.getJSON( "/getAppList", function( data ) {
		  ...
		  $.each( data, function( key, val ) {
		    app_list.push( "<option id='" + val.clientAppID + "'>" + val.appTypeName + "</option>" );
		   	choices = JSON.parse(val.choices);
		   	choice_list = [];
		   	$.each(choices, function(key2, val2){
		   		choice_list.push("<input id='"+key2+"' type='checkbox'><label for='"+key2+"'>"+val2.qa+"</label><br>")
		   	});
  		...
  		$("#app-list").change(function(){
  			...
  			if ($("#app-list option:selected").attr('id')){
  				$("#"+$("#app-list option:selected").attr('id')+"_choices").show();
  			}
  		});
	}
	);